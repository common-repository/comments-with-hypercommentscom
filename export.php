...
<item>
<title><?php echo apply_filters('the_title_rss', $p->post_title); ?></title>
<link><?php $parse = parse_url($p->guid); echo str_replace($parse['scheme'].'://'.$parse['host'], get_option('home'), $p->guid) ?></link>
...
<content:encoded><?php echo hc_export_wxr_cdata( apply_filters('the_content_export', $p->post_content) ); ?></content:encoded>
<dc:thread_identifier><?php echo $p->ID . ' ' . $p->guid; ?></dc:thread_identifier>
<wp:post_id><?php echo $p->ID; ?></wp:post_id>
<wp:post_date_gmt><?php echo $p->post_date_gmt; ?></wp:post_date_gmt>
<wp:comment_status><?php echo $p->comment_status; ?></wp:comment_status>
<?php
        $commentQ = "SELECT * FROM $wpdb->comments WHERE comment_post_ID=".$p->ID ;
